...
.DEFAULT_LAGS <- c(2L, 3L, 4L, 5L, 6L, 7L, 12L, 14L, 24L, 28L, 30L,
                   52L, 60L, 90L, 168L, 365L)
...
period_detector <- function(lags = NULL, alpha = 0.01, min_observations = 50L) {
  if (is.null(lags)) lags <- .DEFAULT_LAGS
  force(alpha); force(min_observations)
  max_lag <- max(lags)

  function(y, state = NULL) {
    if (is.null(state)) {
      state <- list(buffer = numeric(0), n = 0L, mean = 0.0, var = 0.0,
                    cross = rep(0.0, length(lags)))
    }
    ...
    if (nb > max_lag + 1) state$buffer <- buf[-1]
