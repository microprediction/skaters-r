...
.ACK_D <- c(7.784695709041462e-03, 3.224671290700398e-01, 2.445134137142996e+00, 3.754408661907416e+00)
...
spliced_cdf <- function(d, x) {
  z <- .spliced_z(d, x)
  if (z < d$t_lo) {
    return(d$zeta_lo * .gpd_sf(d$t_lo - z, d$g_lo, d$s_lo))
  }
  if (z > d$t_up) {
    return(1.0 - d$zeta_up * .gpd_sf(z - d$t_up, d$g_up, d$s_up))
  }
...
spliced_logpdf <- function(d, x) {
  ...
  z <- .spliced_z(d, x)
  corr <- if (z < d$t_lo) {
    log(max(d$zeta_lo, 1e-300)) + .gpd_logpdf(d$t_lo - z, d$g_lo, d$s_lo) - (-0.5 * z * z - .LOG_SQRT2PI)
  } else if (z > d$t_up) {
    log(max(d$zeta_up, 1e-300)) + .gpd_logpdf(z - d$t_up, d$g_up, d$s_up) - (-0.5 * z * z - .LOG_SQRT2PI)
  } else {
    log(d$c)
  }
  base + corr
...
.tail_new <- function() {
  list(t = NULL, exc = numeric(0), s1 = 0.0, nx = 0L, r = 0.0, g = 0.0, s = 1.0, since = 0L, run = 0L)
}
...
gpdtails <- function(base, k, level = 0.98, nexc = 500L, warmup = 500L, rate_alpha = 0.002) {
  ...
  function(y, state = NULL) {
    if (is.null(state)) {
      state <- list(
        ...
        tails = lapply(seq_len(k), function(m) {
          list(up = .tail_new(), lo = .tail_new(), warm = numeric(0), n = 0L)
        })
      )
    }
    ...
    for (m in seq_len(k)) {
      ...
      if (is.null(up$t)) {
        ...
        if (length(th$warm) >= warmup) {
          ...
          for (x in w) {
            if (x > up$t) {
              up <- .tail_add(up, x - up$t, nexc)
            } else if (x < lo$t) {
              lo <- .tail_add(lo, lo$t - x, nexc)
            }
          }
          th$n <- length(w)
          up$r <- up$nx / length(w)
        ...
        if (z > up$t) {
          up <- .tail_add(up, z - up$t, nexc)
        } else if (z < lo$t) {
          lo <- .tail_add(lo, lo$t - z, nexc)
        }
      }
      th$up <- up
      th$lo <- lo
    ...
    for (m in seq_len(k)) {
      ...
      out[[m]] <- spliced_new(d, lo$t, up$t, lo$r, up$r, lo$g, lo$s, up$g, up$s)
    }
