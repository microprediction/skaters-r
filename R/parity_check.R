...
#' Run the cross-implementation parity suite
#'
#' The assertion engine behind both test harnesses (`tests/parity.R` and the
#' testthat suite): 55 scenarios checked against the Python-generated
#' vectors at a tolerance of 1e-6, plus the candidate-population structure
#' block. Primarily for development and continuous integration.
#'
#' @param vec_path path to `vectors.json`, generated by the reference
#'   implementation's `parity/gen_vectors.py`.
#' @param verbose print per-scenario results.
#' @return a list with per-scenario failure counts, `values_checked`,
#'   `structure_ok`, and total `fails`.
#' @keywords internal
